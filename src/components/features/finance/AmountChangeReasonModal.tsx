...
import type { AmountChangeReasonCode } from '@/lib/api';

interface AmountChangeReasonModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (payload: { reasonCode: AmountChangeReasonCode; reasonOther?: string }) => Promise<void>;
  isSubmitting?: boolean;
}

export const AmountChangeReasonModal = ({
  isOpen,
  onClose,
  onConfirm,
  isSubmitting = false,
}: AmountChangeReasonModalProps) => {
  const [reasonCode, setReasonCode] = useState<AmountChangeReasonCode | ''>('');
    ...
    >
      <div className="space-y-4">
        <Select
          ...
          value={reasonCode}
          onChange={(event) => {
            const nextReason = event.target.value as AmountChangeReasonCode | '';
